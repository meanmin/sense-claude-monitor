"""
Sleep Monitor Logger
Analyzes audio files for snoring and sleep disorder-related sounds
using the cochl.sense SDK and saves results to daily JSON log files.
...
SOUND_TAGS = {
    ...
    "Throat_clear": {
        "description": "Grunting or coughing sound to clear the throat.",
        "severity": "medium",
        "category": "sleep_disruption",
    },
    "Yawn": {
        "description": "Open-mouthed breath sound often with vocal tone from tiredness.",
        "severity": "info",
...
def save_log(file_path: str, events: List[dict]) -> str:
    """Save analysis results to sleep_log_YYYYMMDD.json."""
    os.makedirs(LOG_DIR, exist_ok=True)

    today = datetime.now().strftime("%Y%m%d")
    log_path = os.path.join(LOG_DIR, f"sleep_log_{today}.json")
